...
| **网页幻灯片与 PPTX 网页化**<br>`cs-web-slides` | 用课程、演讲、路演、产品发布内容，或已有 PPT / PPTX，制作可演示、可分享的网页 deck | 新建 deck 先给 3 个真实封面方向；确认后交付 16:9 `index.html`、`assets/`、键盘 / 触控导航与浏览器内文字编辑 |
| **产品宣传视频**<br>`cs-shotcraft-skill` | 用本地产品项目、线上网址、录屏或截图，把功能与品牌价值讲成一支短片 | 产品检查、镜头映射、分镜、Remotion 成片；带 BGM 与不带 BGM 两版，按需导出剪映工程 |

**怎么选：**要完整讲清一条逻辑，选“小黄 PPT 页面”；已有文章、希望提升阅读理解，选“文章封面与配图”；需要可交互演示、要把 PPTX 网页化或要在浏览器播放，选“网页幻灯片与 PPTX 网页化”；要把产品功能做成有镜头、节奏和声音的发布片，选“产品宣传视频”。
...
- 网页幻灯片：3 个真实封面方向（新建时）→ 16:9 `index.html` 与 `assets/`，支持键盘、触控、进度、浏览器内文字编辑与下载修改版本
- 多页 contact sheet
...
### 网页幻灯片与 PPTX 网页化（第三种能力）

知识课程、路演或产品发布内容需要在浏览器现场演示时，使用 `cs-web-slides`。它不是把静态页面塞进网页，而是交付带导航、动效和浏览器内编辑能力的 16:9 演示。
...
它输出固定 16:9 舞台的 HTML，支持键盘、触控、悬停交互、动画与浏览器内文本编辑；但不输出原生可编辑 PPTX。
