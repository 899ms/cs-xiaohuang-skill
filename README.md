...
先看你想解决什么，再选模式。你不需要先写复杂提示词：给出内容、用途和已有素材，skill 会负责把它们组织成可交付的视觉内容。

| 模式 | 最适合的任务与输入 | 默认交付 |
| --- | --- | --- |
| **小黄 PPT 页面** | 用课程提纲、分享主题、产品说明、工作流或文章，讲清一条完整逻辑 | 先给逐页 `slide blueprint`，再交付 5–12 张 16:9 PNG 页面和整套 `contact sheet` |
| **文章封面与配图** | 用公众号、博客、Newsletter、Notion 或知识长文，把关键判断、关系和转折画出来 | 1 张 21:9 封面 + 3–6 张 16:9 正文图，并附 `shot list` 和 `contact sheet` |
| **品牌 IP 延展** | 用标准图需求、动作表情、三视图、场景、媒介或联名方向，沉淀可复用的小黄资产 | 按用途交付单张或多张资产：标准图、动作 / 表情、三视图、2D / 3D、风格迁移或身份修复 |

**怎么选：**要完整讲清一条逻辑，选“小黄 PPT 页面”；已有文章、希望提升阅读理解，选“文章封面与配图”；目标是沉淀可复用的小黄资产，选“品牌 IP 延展”。

所有模式都先守住小黄身份：**动作、场景、媒介和画法可以变，角色身份不能变。**内容型交付还遵守两条质量线：一张图只解释一个核心关系；小黄必须完成解释动作，不能只是角落装饰。
