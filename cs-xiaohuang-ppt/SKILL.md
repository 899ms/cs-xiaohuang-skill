...
description: "将中文文章、课程笔记、演讲提纲、产品说明、工作流或粗略想法，转成以小黄（有温度）为叙事主角的中文轻手绘 PPT-style 整页图像 deck，或生成文章的 21:9 封面与 16:9 正文配图。用户提到小黄 PPT、小黄课件、小黄演示稿、小黄幻灯片、文章配图、公众号配图、博客配图或希望为文章生成小黄插图时使用。默认输出 PNG 页面、contact sheet 与 blueprint / shot list；不输出可编辑 PPTX。"
metadata:
  author: "陈硕"
...
把内容整理成一套统一、可交付的小黄轻手绘 PPT-style 页面图，或一组服务于文章阅读的封面与正文配图。目标不是套模板，而是让小黄用一个个具体动作解释读者真正需要理解的判断、关系和变化。

## 交付边界

- 所有交付都是完整 raster PNG 页面图；PPT 标准页面和文章正文图为 16:9，推荐 1920×1080。
...
3. **选择输出模式。** 用户要演讲、课件或演示稿时选择 **deck**；用户要文章、公众号、博客或 Newsletter 配图时选择 **article illustration**。文章模式读取 `references/article-illustrations.md`，先给 shot list；deck 模式读取 `references/narrative-planning.md`，先定整套叙事。两种模式中每页 / 每张图只能有一个主观点。
...
完成后简短报告：输出目录和 contact sheet 路径、deck 页数或文章配图数量、采用的假设、已完成的质量检查，以及仍存在的文字或身份漂移风险。
